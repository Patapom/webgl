...
namespace FBXImporter
{
	...
	public:		// METHODS
		...
		void		Load( System::String^ _FileName )
		{
			...
			try
			{
				String^	ManifestFileName = m_SceneFileName + ".manifest";
				if ( System::IO::File::Exists( ManifestFileName ) )
				{
					m_Manifest = gcnew Manifest( this, ManifestFileName );
					m_Manifest->Load();
					m_Manifest->ApplyToScene();	// Apply the manifest data to the scene
				}
			}
			catch ( Exception^ )
			{
		...
		// Creates a manifest file attached to the FBX scene file
		//
		Manifest^		CreateManifest()
		{
			if ( m_SceneFileName == nullptr )
				throw gcnew Exception( "You cannot create a manifest before the FBX scene is loaded !" );
